...
import com.punchcyber.pakhet.datatypes.packet.{IpPacketRecord, PacketFeatureStat, UdpPacketRecord}
...
class UdpPacketSummary(timeout:        Float,
                       /*tuple:          Tuple,*/
                       packetRecord:   UdpPacketRecord) extends IpPacketSummary(timeout/*,tuple*/,packetRecord) {
    /**/
    var id_orig_p: Int = packetRecord.id_orig_p
    var id_resp_p: Int = packetRecord.id_resp_p
    val service: String = packetRecord.service
    ...
    def add_packet(record: UdpPacketRecord/*,_tuple: Tuple*/): Option[UdpPacketRecord] = {
    ...
    override def getHBasePut: Put = {
        ...
        val first_packet: UdpPacketRecord = record_buffer.first.asInstanceOf[UdpPacketRecord]
        id_orig_h = first_packet.id_orig_h
        id_resp_h = first_packet.id_resp_h
        id_orig_p = first_packet.id_orig_p
        id_resp_p = first_packet.id_resp_p
        ...
        if(first_packet.id_orig_p < first_packet.id_resp_p) {
            id_orig_h = first_packet.id_resp_h
            id_resp_h = first_packet.id_orig_h
            id_orig_p = first_packet.id_resp_p
            id_resp_p = first_packet.id_orig_p
        ...
        while(iter.hasNext) {
            val packet: UdpPacketRecord = iter.next.asInstanceOf[UdpPacketRecord]
